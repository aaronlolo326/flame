...
def _should_log_debug(module) -> bool:
    ...
    if not bool(getattr(cfg, "debug_ttt_logs", False)):
        return False
    try:
        ...
        pass
    if not hasattr(module, "_debug_forward_calls"):
        module._debug_forward_calls = 0
    module._debug_forward_calls += 1
    every = max(1, int(getattr(cfg, "debug_ttt_log_every", 20)))
    return module._debug_forward_calls == 1 or (module._debug_forward_calls % every == 0)
...
def _tensor_stats(name: str, tensor: Optional[torch.Tensor]) -> str:
    ...
    with torch.no_grad():
        ...
        if not t.is_floating_point():
            t64 = t.to(torch.int64)
            return f"{name}[shape={shape}, dtype={dtype}, min={t64.min().item()}, max={t64.max().item()}]"
...
class Qwen3MLP(nn.Module):
    ...
    def forward(self, x, t: Optional[torch.Tensor] = None):  # TTT: added t param
        ...
        d_down_proj_sum = torch.cat(
            [
                d_down_proj.new_zeros(bs, 1, d_down_proj.shape[-2], d_down_proj.shape[-1]),
                (d_down_proj * self.ttt_lr).cumsum(dim=1),
            ],
            dim=1,
        )
