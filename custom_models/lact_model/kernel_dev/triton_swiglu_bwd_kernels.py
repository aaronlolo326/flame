...
def _swiglu_three_bmm_kernel(
    ...
    s_x_n,
    s_x_k,
    # strides for outputs [B, 2M, N] or [T, 2M]
    s_dy0_dy2_b,
    s_dy0_dy2_m,
    s_dy0_dy2_n,
    ...
    # Bases for this batch/doc (weights always per-doc)
...
def swiglu_backward_three_bmm_varlen_triton(
    ...
    G, M_times_2, K = W0_W2.shape
    M = M_times_2 // 2
    T = X.shape[0]

    DY0_DY2 = torch.empty((T, M_times_2), device=X.device, dtype=X.dtype)
    Hidden = torch.empty((T, M), device=X.device, dtype=X.dtype)

    out_dtype_str = (
        "float32" if X.dtype == torch.float32
        else "bf16" if X.dtype == torch.bfloat16 else "fp16"
    )
    ...
    def grid(meta):
        return (triton.cdiv(M, meta["BLOCK_M"]), triton.cdiv(max_sl, meta["BLOCK_N"]), G)
    ...
    _swiglu_three_bmm_kernel[grid](
        ...
        max_sl,
        K,
        # strides for W0_W2 [G, 2M, K] — per-doc
        W0_W2.stride(0), W0_W2.stride(1), W0_W2.stride(2),
        # strides for W1 [G, K, M] — per-doc
        W1.stride(0), W1.stride(1), W1.stride(2),
        # strides for X, V [T, K] — packed
        0, X.stride(0), X.stride(1),
        # strides for DY0_DY2 [T, 2M] — packed
        0, DY0_DY2.stride(1), DY0_DY2.stride(0),
        # strides for Hidden [T, M] — packed
        0, Hidden.stride(1), Hidden.stride(0),
        out_dtype=out_dtype_str,
        IS_VARLEN=True,
    )
...
def _reference_varlen_padded(W0_W2, W1, X, V, cu_seqlens):
    """Pad -> batched kernel -> unpack."""
    ...
    DY0_DY2_b, Hidden_b = swiglu_backward_three_bmm_triton(W0_W2, W1, X_pad, V_pad)
    return (
...
if __name__ == "__main__":
    ...
    # ===== Correctness =====
    print("=" * 60)
    print("swiglu_backward_three_bmm varlen correctness")
    print("=" * 60)

    args = (W0_W2, W1, X, V, cu_seqlens)
    test_correctness(
    ...
    print()
    print("=" * 60)
    print("swiglu_backward_three_bmm varlen benchmark")
    print("=" * 60)
    ...
    benchmark(
        lambda *a: swiglu_backward_three_bmm_varlen_triton(
            *a, eff_lens=eff_lens, bos_arr=bos_arr, max_sl=max_sl_val,
        ),
        lambda *a: swiglu_backward_three_bmm_triton(*a),
        args, (W0_W2, W1, X_pad, V_pad),
    )
    ...
    for ci in range(max_chunks):
        chunk_cu, idx, _ = get_chunk_info(cu_seqlens, chunk_size, ci)
        if len(idx) == 0:
            break
        ref_dy, ref_h = swiglu_backward_three_bmm_varlen_triton(
            W0_W2, W1, X[idx], V[idx], chunk_cu,
        )
        test_dy, test_h = swiglu_backward_three_bmm_varlen_triton(
            W0_W2, W1, X, V, cu_seqlens, chunk_size=chunk_size, chunk_idx=ci,
        )
        diff_dy = (test_dy[idx] - ref_dy).abs().max().item()
        diff_h = (test_h[idx] - ref_h).abs().max().item()
        print(f"  chunk_idx={ci}: DY0_DY2 max_diff={diff_dy:.2e}, Hidden max_diff={diff_h:.2e}, n_tokens={len(idx)}")
        assert diff_dy == 0.0 and diff_h == 0.0, f"chunk_idx={ci} not exact match!"
