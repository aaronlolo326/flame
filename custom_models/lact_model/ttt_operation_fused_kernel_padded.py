...
def prenorm_block_causal_lact_swiglu_fused_kernel_triton(
    ...
    """
    Varlen wrapper around the padded prenorm TTT kernel.
    Unpacks packed inputs, pads to max_doc_len, runs padded kernel, repacks output.
    Uses the old (proven) batched kernels for correctness reference.
    """
    nh = w0.shape[0]
    num_docs = cu_seqlens.shape[0] - 1
    packed_len = q.shape[1]
    d = w0.shape[2]
    max_doc_len = packed_len
    B = nh * num_docs

    # Build packed->padded index mapping (no .item(), compile-friendly)
    tok_idx = torch.arange(packed_len, device=q.device)
    tok_doc = torch.searchsorted(cu_seqlens[1:].long(), tok_idx.long(), right=True)
    tok_pos = tok_idx - cu_seqlens[tok_doc]

    # Zero lr at each doc's last-chunk positions so the padded kernel
    # treats them as apply-only (no grad+update), regardless of max_doc_len.
    doc_lens = cu_seqlens[1:] - cu_seqlens[:-1]
    nchunks = (doc_lens + chunk_size - 1) // chunk_size
    ...
    def _up(t):
        """[nh, packed_len, *] -> [B, max_doc_len, *] zero-padded via scatter."""
        rest = t.shape[2:]
        out = t.new_zeros(nh, num_docs, max_doc_len, *rest)
        out[:, tok_doc, tok_pos] = t
        return out.reshape(B, max_doc_len, *rest)

    out_pad = _prenorm_padded(
        w0.repeat_interleave(num_docs, 0),
        w1.repeat_interleave(num_docs, 0),
        w2.repeat_interleave(num_docs, 0),
        _up(q), _up(k), _up(v),
        _up(lr0), _up(lr1), _up(lr2),
        chunk_size=chunk_size, use_muon=use_muon,
        momentum=_up(momentum) if momentum is not None else None,
    ).reshape(nh, num_docs, max_doc_len, d)

    # Repack: [nh, num_docs, max_doc_len, d] -> [nh, packed_len, d]
    output = out_pad[:, tok_doc, tok_pos]
    return output
