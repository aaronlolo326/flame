...
class E2ETTTConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        e2e_ttt_groups: Optional[List[Dict[str, Any]]] = None,
        ...
        **kwargs,
        ...
        if e2e_ttt_groups is None:
            self.e2e_ttt_groups = None
        elif isinstance(e2e_ttt_groups, (list, tuple)):
            normalized_groups: List[Dict[str, Any]] = []
            for idx, spec in enumerate(e2e_ttt_groups):
                if isinstance(spec, dict):
                    normalized_groups.append(dict(spec))
                else:
                    try:
                        normalized_groups.append(dict(spec))
                    except Exception as exc:
                        raise ValueError(
                            f"e2e_ttt_groups[{idx}] must be dict-like, got {type(spec).__name__}"
                        ) from exc
            self.e2e_ttt_groups = normalized_groups
        else:
            raise ValueError("e2e_ttt_groups must be a list of dict-like group specs or None")
...
class E2ETTTV2Config(E2ETTTConfig):
    model_type = "e2e_multi_level"


class E2EMultiLevelConfig(E2ETTTV2Config):
    model_type = "e2e_multi_level"
