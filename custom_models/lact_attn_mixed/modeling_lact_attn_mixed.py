...
from .layer_lact_attn_mixed import LaCTSWIGLULayer
...
class LaCTAttnMixedBlock(nn.Module):

    def __init__(self, config: LaCTAttnMixedConfig, layer_idx: int):
        super().__init__()
        ...
        self.attn = LaCTSWIGLULayer(
            ...
            use_ttt_depth_mix=config.use_ttt_depth_mix,
            depth_mix_dim=config.depth_mix_dim,
            depth_mix_detach_cache=config.depth_mix_detach_cache,
            depth_mix_init_gate_bias=config.depth_mix_init_gate_bias,
        )
    ...
    def forward(
        ...
        **kwargs: Unpack[Any],
        ...
        hidden_states, attentions, past_key_values, depth_info = self.attn(
        ...
        depth_payload = depth_info if depth_info is not None else hidden_states.new_empty(0)
        outputs = (hidden_states, depth_payload)
...
class LaCTAttnMixedPreTrainedModel(PreTrainedModel):

    config_class = LaCTAttnMixedConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["LaCTAttnMixedBlock"]
    ...
    def _init_weights(
        ...
        if isinstance(module, LaCTSWIGLULayer):
            ...
            if module.depth_gate_proj is not None:
                nn.init.zeros_(module.depth_gate_proj.weight)
                nn.init.constant_(
                    module.depth_gate_proj.bias, module.depth_mix_init_gate_bias
                )


class LaCTAttnMixedModel(LaCTAttnMixedPreTrainedModel):

    def __init__(self, config: LaCTAttnMixedConfig) -> LaCTAttnMixedModel:
        ...
        self.layers = nn.ModuleList(
            [
                LaCTAttnMixedBlock(config, layer_idx)
    ...
    def forward(
        ...
        **kwargs: Unpack[Any],
        ...
        next_cache = None
        depth_cache = {"keys": [], "values": []}
        use_depth_mix = bool(self.config.use_ttt_depth_mix)
        if use_depth_mix and getattr(self.config, "depth_mix_block_size", 0) > 0:
            logger.warning_once(
                "`depth_mix_block_size>0` is not enabled yet; fallback to per-layer cache."
            )
        use_layer_checkpoint = self.gradient_checkpointing and self.training
        if use_layer_checkpoint and use_depth_mix:
            logger.warning_once(
                "`use_ttt_depth_mix=True` currently runs without layer checkpointing."
            )
        ...
        for layer in self.layers:
            ...
            depth_info = layer_outputs[1]
            if use_depth_mix and isinstance(depth_info, dict):
                depth_cache["keys"].append(depth_info["key"])
                depth_cache["values"].append(depth_info["value"])
...
class LaCTAttnMixedForCausalLM(LaCTAttnMixedPreTrainedModel, GenerationMixin):

    _tied_weights_keys = {"lm_head.weight": "model.embeddings.weight"} # ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = LaCTAttnMixedModel(config)
        self.vocab_size = config.vocab_size
