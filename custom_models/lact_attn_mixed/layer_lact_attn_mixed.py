...
from ..lact_model.ttt_operation import (
    ...
    l2_norm,
    # prenorm_block_causal_lact_swiglu_varlen,
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.num_fw_heads > 0:
            ...
            if cu_seqlens is not None and True:
                if self.use_fused_kernel:
                    raise NotImplementedError("varlen + fused LaCT kernel not implemented yet")
                else:
                    # fw_x = prenorm_block_causal_lact_swiglu_varlen(
                    #     fw_w0,
                    #     fw_w1,
                    #     fw_w2,
                    #     fast_q,
                    ...
                    #     momentum=momentum,
                    # )
                    fw_x = prenorm_block_causal_lact_swiglu_varlen_bucketed(
                        fw_w0,
                        fw_w1,
                        fw_w2,
                        fast_q,
                        ...
                        use_muon=self.use_muon,
                        momentum=momentum,
                        max_segments_per_bucket=8,        # 先保守，防 OOM
                    )
            else:
                # [b * nh, s, d_ttt_head]
                if self.ttt_prenorm:
