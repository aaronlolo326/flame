...
class HybridQwen3LaCTBranch(nn.Module):
    def __init__(self, config: HybridQwen3LaCTConfig, layer_idx: int) -> None:
        ...
        self.ttt_inner_steps = config.ttt_inner_steps
        ...
        if self.ttt_prenorm:
            if use_fused:
                fw_x = prenorm_block_causal_lact_swiglu_fused_kernel_triton(
                    ...
                    chunk_size=self.lact_chunk_size, ttt_inner_steps=self.ttt_inner_steps,
...
class HybridQwen3LaCTModel(HybridQwen3LaCTPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Treat an all-ones mask as "no padding mask" so CUDA eval can stay on the
        # FlashAttention path instead of falling back to dense masked attention.
