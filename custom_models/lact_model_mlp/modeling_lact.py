...
from .layer_lact_swiglu import LaCTMLPSWIGLULayer
...
class LaCTMLPBlock(nn.Module):

    def __init__(self, config: LaCTMLPSWIGLUConfig, layer_idx: int):
        super().__init__()
...
class LaCTMLPPreTrainedModel(PreTrainedModel):

    config_class = LaCTMLPSWIGLUConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["LaCTMLPBlock"]
    ...
    def _init_weights(
        ...
        if rescale_prenorm_residual:
            ...
            logger.info(
                f"in PreTrainedModel initialize fast weights for LaCTMLPSWIGLULayer"
            )
            # init w0, w1, w2
...
class LaCTMLPModel(LaCTMLPPreTrainedModel):

    def __init__(self, config: LaCTMLPSWIGLUConfig) -> LaCTMLPModel:
...
class LaCTMLPForCausalLM(LaCTMLPPreTrainedModel, GenerationMixin):

    _tied_weights_keys = {"lm_head.weight": "model.embeddings.weight"} # ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = LaCTMLPModel(config)
        self.vocab_size = config.vocab_size
