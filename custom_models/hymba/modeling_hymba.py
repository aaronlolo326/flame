...
class HymbaRMSNorm(nn.Module):
    ...
    def reset_parameters(self):
        ...
        # print (f"resetted {self.weight=}")

    def forward(self, hidden_states):
        # print (f"1 {hidden_states=} {hidden_states.shape}") # [bsz, seq_len, hidden_size]
        input_dtype = hidden_states.dtype
        hidden_states = hidden_states.to(torch.float32)
        # print (f"2 {hidden_states=} {hidden_states.shape}") # [bsz, seq_len, hidden_size]
        variance = hidden_states.pow(2).mean(-1, keepdim=True)
        hidden_states = hidden_states * torch.rsqrt(variance + self.variance_epsilon)
        # print (f"3 {hidden_states=} {hidden_states.shape}") # [bsz, seq_len, hidden_size]
        # print (f"4 {self.weight=}")
...
class HymbaAttention(nn.Module):
    ...
    def __init__(self, config: MyHymbaConfig, layer_idx: Optional[int] = None, reuse_kv=False, output_hidden_size=None, attn_only_wo_proj=False):
        super().__init__()
        self.config = config
...
class HymbaFlashAttention2(HymbaAttention):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._flash_attn_uses_top_left_mask = not False # not is_flash_attn_greater_or_equal_2_10()
    ...
    def forward(
        ...
        # print (use_sliding_windows)
        # print((
        #         _flash_supports_window_size,
        #         getattr(self.config, "sliding_window", None) is not None,
        #         kv_seq_len > (self.config.sliding_window + self.config.num_memory_tokens if self.config.num_memory_tokens > 0 else self.config.sliding_window),
        #         use_swa
        # ))
        ...
        # print (f"{use_sliding_windows=}, {swa_processed_flag=}") True, False
...
class HymbaSdpaAttention(HymbaAttention):
    ...
    def forward(
        ...
        # Reference: https://github.com/pytorch/pytorch/issues/`11`2577.
...
class HymbaBlock(nn.Module):
    ...
    def __init__(self, config: MyHymbaConfig, layer_idx, reuse_kv=None):
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
        self.hidden_size = config.hidden_size
        ...
        # A = torch.arange(1, self.ssm_state_size + 1, dtype=torch.bfloat16)[None, :]
...
class HymbaMLP(nn.Module):
    def __init__(self, config: MyHymbaConfig):
        super().__init__()
        # self.config = config
...
class HymbaSparseMoeBlock(nn.Module):
    ...
    def __init__(self, config: MyHymbaConfig, num_experts: int, num_experts_per_tok: int):
        super().__init__()
        self.hidden_dim = config.hidden_size
        self.ffn_dim = config.intermediate_size
...
class HymbaDecoderLayer(nn.Module):
    def __init__(self, config: MyHymbaConfig, num_experts: int, layer_idx: int, reuse_kv: bool = False):
        super().__init__()
    ...
    def forward(
        ...
        # print (f"before {hidden_states=}")
        hidden_states = self.input_layernorm(hidden_states)
        # print (f"after {hidden_states=}")
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (nn.Linear, nn.Conv1d)):
            ...
            if module.padding_idx is not None:
                # Avoid integer-index selection here because sharded tensors can
                # lower it to aten.select.int, which may not have a strategy.
                module.weight.data[module.padding_idx : module.padding_idx + 1].zero_()
        elif hasattr(module, "reset_parameters"):
            module.reset_parameters()
        # if isinstance(module, HymbaRMSNorm):
        #     print (module, hasattr(module, "reset_parameters"))
...
class MyHymbaModel(HymbaPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`HymbaDecoderLayer`]

    Args:
        config: MyHymbaConfig
    """

    def __init__(self, config: MyHymbaConfig):
        super().__init__(config)
        # if (
        #     config.attn_implementation_new == "flash_attention_2"
        #     and config.sliding_window is not None
        #     and not _flash_supports_window_size
        # ):
        #     logger.warning_once(
        #         "Requested sliding-window attention with flash_attention_2, but the installed flash-attn "
        #         "does not expose `window_size`. Falling back to `sdpa` so training respects "
        #         f"`sliding_window={config.sliding_window}`."
        #     )
        #     config.attn_implementation_new = "sdpa"
        ...
        print (f"{self.padding_idx=}")
        if self.padding_idx is not None and self.padding_idx in {config.eos_token_id, config.bos_token_id}:
            logger.warning_once(
                "pad_token_id overlaps with a semantic special token (BOS/EOS). "
                "Disabling nn.Embedding padding_idx so that token stays trainable."
            )
    ...
    def forward(
        ...
        # print (f"{self._attn_implementation=}")
...
class MyHymbaForCausalLM(HymbaPreTrainedModel):
    ...
    def __init__(self, config: MyHymbaConfig):
        super().__init__(config)
        self.config = config
        self.model = MyHymbaModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        logits = None
        if not use_liger_kernel or check:
            if calc_logits_for_entire_prompt:
                # print ("Calculating logits for the entire prompt.")
                logits = self.lm_head(hidden_states)
            else:
                # print ("Not calculating logits for the entire prompt.")
        ...
        if labels is not None:
            ...
            if use_liger_kernel:
                ...
                hidden_states = hidden_states[..., :-1, :].contiguous().view(-1, self.config.hidden_size)
                # print (f"{hidden_states.shape=}, {shift_labels.shape=}")
                loss_liger = lce(self.lm_head.weight, hidden_states, shift_labels)
            if not use_liger_kernel or check:
                ...
                loss_torch = loss_fct(shift_logits, shift_labels)
                if check:
                    torch.testing.assert_close(loss_liger, loss_torch, rtol=1e-4, atol=1e-6)
        loss = loss_liger if loss_liger is not None else loss_torch
