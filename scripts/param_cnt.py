...
def main(job_config: JobConfig):
    ...
    ## Below is for GDN-hybrid, which requires transformer=4.53.3
    # import json
    ...
    num_nondistinct_params, nparams_embedding, num_flops_per_token = get_nparams_and_flops(
        model, model_config, job_config.training.context_len
    )
    print(f"Number of non-distinct model parameters = {num_nondistinct_params:,}")
    if model_config.tie_word_embeddings:
        print("Model ties input and output word embeddings, so")
        print(f"Number of distinct parameters = {num_nondistinct_params - nparams_embedding:,}")
    else:
