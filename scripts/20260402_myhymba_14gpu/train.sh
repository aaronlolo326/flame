...
available_nodes=(12 6) # first is master node
...
elif [ "$MODEL_TYPE" = "my_hymba" ]; then
  data_parallel_replicate_degree=${NNODE_TOTAL} # ${NNODE_TOTAL}
  data_parallel_shard_degree=1
fi
...
NNODE=${NNODE} NGPU=${NGPU} LOG_RANK=${local_rank} bash train.sh \
  ...
  --training.data_parallel_shard_degree ${data_parallel_shard_degree} \
