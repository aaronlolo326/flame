...
def sample_chunk(tokens, context_length):
    if len(tokens) <= context_length:
        return tokens
    start = torch.randint(
        0, len(tokens) - context_length + 1, (1,)
    ).item()
    return tokens[start:start + context_length]
    ...
    def __iter__(self):
        ...
        while True:
            ...
            if not self.is_tokenized:
                samples = self.tokenize(self.data)
            else:
                samples = self.data
            for sample in samples:
                ...
                # print (f"{len(sample)=}")
                # print (sample[:20], sample[-20:])
                # print (self.tokenizer.decode(sample[:20]), " ... ", self.tokenizer.decode(sample[-20:]))
                # print ()
                # breakpoint()
                if self.sample_trunc_seq:
                    sample = sample_chunk(sample, self.seq_len)
        ...
        if not self.varlen:
            ...
            if needs_padding:
                ...
                if self.tokenizer is None:
                    raise ValueError(
                        f'You are attempting to pad samples but self.tokenizer is None'
                    )
...
#     def __init__(self, zarr_array, seq_len):
#         self.zarr_array = zarr_array
#         self.seq_len = seq_len
#         self.total_tokens = zarr_array.shape[0]
...
#         tokens = self.zarr_array[idx * self.seq_len:(idx + 1) * self.seq_len + 1]
#         input_ids = torch.tensor(tokens[:-1], dtype=torch.long)
#         labels = torch.tensor(tokens[1:], dtype=torch.long)
#         return {"input_ids": input_ids, "labels": labels}

def zarr_sample_generator(zarr_array, seq_len):
    total_tokens = zarr_array.shape[0]
    num_samples = (total_tokens - 1) // seq_len
    for idx in range(num_samples):
        start = idx * seq_len
        end = start + seq_len + 1
        tokens = zarr_array[start:end]
        yield {
...
def build_dataset(
    ...
    path = tokenized_dataset_dir if tokenized_dataset_dir is not None else dataset
    assert dataset is None or tokenized_dataset_dir is None, f"{tokenized_dataset_dir=}, {dataset=}"
    if len(path.split(',')) == 1:
        ...
        if tokenized_dataset_dir is not None:
            ...
            logger.info(f"to_iterable_dataset ({min_num_shards=})")
            ...
            dataset = load_dataset(
                path=path, name=dataset_name, split=dataset_split,
                data_dir=data_dir, data_files=data_files, streaming=streaming,
                num_proc=num_workers if not streaming else None,
                # cache_dir='/nfs-export/hei/.cache/huggingface/datasets'
            )
